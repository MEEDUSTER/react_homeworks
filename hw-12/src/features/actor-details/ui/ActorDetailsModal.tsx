...
// 2010s Tough Guy / Boy Quotes Array for Jason Statham
...
export function ActorDetailsModal({ actorId, onClose }: ActorDetailsModalProps) {
  ...
  // State for Statham interactive quote section
  ...
  // Check if current actor is Jason Statham (id 976)
  const isStatham = actorId === 976 || details?.id === 976 || details?.name.toLowerCase().includes('jason statham');
    ...
    <Modal isOpen={!!actorId} onClose={onClose}>
      {isDetailsLoading || isCreditsLoading ? (
        <div className="flex flex-col justify-center items-center h-80 gap-4">
          ...
          <p className="text-sm text-slate-400 font-medium">Завантаження профілю...</p>
        </div>
      ) : isDetailsError || !details ? (
        <div className="text-center text-rose-400 py-12 text-lg">
          Помилка завантаження даних актора.
        </div>
      ) : (
        <div className="space-y-8">
          {/* STATHAM SPECIAL HEADER BANNER ("2 КАРТИ І 2 СТВОЛА") */}
          {isStatham && (
            <div className="relative overflow-hidden rounded-2xl p-6 lock-stock-card border border-amber-500/40 shadow-2xl">
              {/* Card Watermark */}
              <div className="absolute right-4 -bottom-6 text-[120px] text-amber-500/10 font-cinzel font-black select-none pointer-events-none">
                ♠ 🔫
              </div>
              ...
              <div className="relative z-10 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <h1 className="text-3xl sm:text-4xl font-black font-cinzel gold-gradient-text tracking-wide">
                      ДЖЕЙСОН СТЕТХЕМ
                    </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-8 items-start">
            ...
            <div className="md:col-span-8 space-y-6">
              ...
              <div className="space-y-2 p-4 rounded-2xl bg-white/[0.02] border border-white/5">
                ...
                <p className="text-sm text-slate-300 leading-relaxed max-h-48 overflow-y-auto pr-2 text-justify">
          ...
          {/* JASON STATHAM SPECIAL: 2010s BOY QUOTES SHOWCASE */}
          {isStatham && (
            <div className="space-y-6 pt-4 border-t border-amber-500/20">
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pb-1 max-w-full">
                  {[
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition cursor-pointer ${selectedCategory === tab.id
              ...
              <div className="relative p-6 sm:p-8 rounded-3xl lock-stock-card border border-amber-500/40 shadow-xl overflow-hidden group">
                ...
                <div className="relative z-10 max-w-2xl mx-auto text-center space-y-4">
                  ...
                  <blockquote className="text-xl sm:text-2xl font-extrabold text-amber-100 leading-snug font-sans tracking-wide italic">
                    "{currentQuote.quote}"
                  </blockquote>
              ...
              {/* ALL QUOTES GRID (PLAYING CARDS STYLE) */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredQuotes.map((item) => (
                  <div
                    key={item.id}
                    onClick={() => setActiveQuoteIndex(STATHAM_QUOTES.findIndex(q => q.id === item.id))}
                    className={`relative p-5 rounded-2xl transition-all duration-300 cursor-pointer flex flex-col justify-between border ${currentQuote.id === item.id

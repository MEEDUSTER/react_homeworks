...
import { Outlet, NavLink, Link, useNavigate, useLocation, Form } from 'react-router-dom';
import { Button } from '@/shared/ui/button';
import { Clapperboard, ShieldAlert, Heart, Home as HomeIcon, User, LogOut, LogIn } from 'lucide-react';
...
export default function MainLayout() {
  ...
  return (
    <div className="relative min-h-screen bg-[#090d16] text-slate-100 overflow-x-hidden selection:bg-blue-500 selection:text-white flex flex-col">
      ...
      {/* Floating Apple Glass Navigation Header */}
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2.5 bg-white/5 p-1.5 pl-3 rounded-full border border-white/10">
                ...
                <Form action="/logout" method="post">
                  <button
                    type="submit"
                    className="inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full bg-rose-500/20 text-rose-300 hover:bg-rose-500 hover:text-white transition duration-200 border border-rose-500/30 cursor-pointer"
      ...
      <footer className="mt-auto border-t border-white/5 py-8 text-center text-xs text-slate-500">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p>© {new Date().getFullYear()} Actor Spotlight. React Router v8 & Auth TMDB Edition.</p>
